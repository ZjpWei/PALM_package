...
######## Utility file for PALM ########
palm_tune <- function(summary.stats,
  ...
  feature.ID <- rownames(summary.stats[[1]]$est) # sort(unique(feature.ID))
  K <- length(feature.ID)
  # ##
  # for(l in 1:length(summary.stats)){
  #   summary.stats[[l]]$est <- summary.stats[[l]]$est[feature.ID,,drop=FALSE]
  #   summary.stats[[l]]$stderr <- summary.stats[[l]]$stderr[feature.ID,,drop=FALSE]
  # }
  ...
  for(d in study.ID){
    ...
    nonna.id <- !is.na(summary.stats[[d]]$est[,1])
    taxa.vec.tmp[rownames(summary.stats[[d]]$est)[nonna.id]] <- TRUE
  ...
  for(cov.name in cov.int.ID){
    ...
    for(d in study.ID){
      if(cov.name %in% colnames(summary.stats[[d]]$est)){
        nonna.id <- !is.na(summary.stats[[d]]$est[,cov.name])
        summary.stat.study[[d]] <- list(est = summary.stats[[d]]$est[nonna.id,cov.name],
                                        cov = diag((summary.stats[[d]]$stderr[nonna.id,cov.name])^2),
                                        n = summary.stats[[d]]$n)
...
Get_lasso_pre = function(summary.stat.study, feature.ID, feature.set, study.ID, K){
  ...
  for(d in study.ID){
    ...
    if(d == study.ID[1]){
      ...
      X.col.2 <- Matrix::bdiag(X.col.2, Sigma.chol)
  ...
  lasso.mat <- list(X.enlarge = X.col, X.col.1 = X.col.1,
                    Y.enlarge.1 = Y.enlarge.1, Y.enlarge.2 = Y.enlarge.2,
                    taxa.mat = taxa.mat, k.l = k.l, N = N, mu.len = mu.len,
                    nonempty.id = nonempty.id, empty.id = empty.id,
                    feature.ID = feature.ID, feature.set = feature.set, study.ID = study.ID)
...
byabess_AA <- function(summary.stat.study, lasso.mat, delta, support.size){
  ...
  #=== glmtlp model ===#
  # suppressMessages(
  #   result <- glmtlp::glmtlp(X = as.matrix(X.enlarge),
  #                            y = Y.enlarge,
  #                            family = "gaussian",
  #                            penalty = "l0",
  #                            kappa = support.size,
  #                            tol = 1e-2,                  # looser tolerance
  #                            cd.maxit = 1e2,                 # avoid excessive inner loops
  #                            standardize = FALSE)
  # )
  # fit.coef <- result$beta[1:mu.len,1]
  # zero.fit <- rep(0, length(empty.id))
  # names(zero.fit) <- empty.id
  # nonzero.fit <- fit.coef
  # names(nonzero.fit) <- nonempty.id
  # mu.fit <- c(nonzero.fit, zero.fit)[feature.ID]
  # q_loss <- sum( (Y.enlarge - X.enlarge %*% result$beta[,1])^2 )

  # Matrix initialization optimization
  if(L > 1){
    beta.fit <- matrix(fit.coef, ncol = L, nrow = mu.len)
    beta.fit <- beta.fit[match(feature.ID, nonempty.id), , drop = FALSE]
  }else{
    beta.fit <- mu.fit
  }
  return(list(fit.coef = result$beta, mu.fit = mu.fit, beta.fit = beta.fit,
              gic = result$tune.value, delta = delta, q_loss = q_loss, N = lasso.mat$N))
}

# Calculate GIC
GIC.cal <- function(result, tune.type = c("BIC", "HBIC", "KBIC", "EBIC")){
  tune.type <- match.arg(tune.type)
  df <- sum(result$fit.coef!=0)
  K <- length(result$fit.coef)
  L <- length(result$delta)
  P <- K
  if(tune.type == "BIC"){
    return(result$q_loss + df * log(result$N)/result$N )
  }else if(tune.type == "KBIC"){
    return(result$q_loss + df * log(max(exp(1), log(length(result$fit.coef)))) * log(result$N)/result$N )
  }else if(tune.type == "HBIC"){
    return(result$q_loss + df * log(length(result$fit.coef)) * log(log(result$N))/result$N)
...
meta.analysis <- function(summary.stat.study = summary.stat.study,
  ...
  if(tune.path == "sequence"){
    ...
    for(s.lambda in support.sizes){
      ...
      tmp.result <- tmp.subset$tmp.result
      GIC.result <- tmp.subset$GIC.result
      tmp.result$search.loc <- tmp.subset$search.loc
    ...
    if(output.best.one){
      ...
      # beta <- min.result$beta.fit
      # alpha <- min.result$alpha.fit
      ...
      gic <- NULL
      coef <- NULL
      # beta <- array(NA, dim = c(length(tune.set[[1]]$mu.fit), length(tune.set), L),
      #               dimnames = list(names(tune.set[[1]]$mu.fit), 1:length(tune.set),
      #                               1:L))
      # alpha <- array(NA, dim = c(length(tune.set[[1]]$mu.fit), length(tune.set), L),
      #                dimnames = list(names(tune.set[[1]]$mu.fit), 1:length(tune.set),
      #                                1:L))
      delta <- NULL
      dev <- NULL
      ...
      for(l in 1:length(tune.set)){
        search.loc <- rbind(search.loc, tune.set[[l]]$search.loc)
        gic <- c(gic, tune.set[[l]]$gic)
        coef <- cbind(coef, tune.set[[l]]$mu.fit)
        delta <- cbind(delta, tune.set[[l]]$delta)
        # beta[,l,] <- tune.set[[l]]$beta.fit
        # alpha[,l,] <- tune.set[[l]]$alpha.fit
...
search.ref.loc.gsec <- function(summary.stat.study,
  ...
  while(loop.study){
    ...
    s.all.GIC <-  c(s.1.GIC, s.left.GIC, s.right.GIC, s.2.GIC)
    #print(rbind(s.all.prop, s.all.GIC))
