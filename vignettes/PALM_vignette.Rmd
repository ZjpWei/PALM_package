...
The figure below displays the features with $q$-value $≤ 0.05$. Each dot in scatter plot represents the coefficient estimate, with error bars indicating the 95% confidence intervals. The bar plot shows the $-\log_{10}(q\text{-value})$.
...
df.scatter <- palm.df %>% ggplot(aes(x=feature, y=coef)) +
      ...
      geom_point(aes(color = dir), pch = 18, size = 2.5) + ylim(-3, 0.5) +
...
The figure below shows dots representing individual study estimates, with error bars indicating the 95% confidence intervals. Features with a heterogeneity $q$-value $\leq 0.1$ are highlighted with a yellow background, this results demonstrate that PALM achieves good harmonization of features across these two CRC studies, with no heterogeneity detected.

```{r, warning=FALSE, fig.align="center"}
df.plot <- NULL
df.area <- NULL
for(l in palm.df$feature){
    df.plot <- rbind(df.plot, tibble(feature = l, 
...
The following figure displays the features associated with butyrate production with adjusted coefficient $q$-value $\leq 0.05$. Each dot in the scatter plot represents the coefficient estimate, with error bars indicating 95% confidence intervals in individual studies. The features with heterogeneity $q$-value $\leq 0.1$ are highlighted with a yellow background, this results demonstrate that PALM achieves good harmonization of features across studies, with no heterogeneity detected.
